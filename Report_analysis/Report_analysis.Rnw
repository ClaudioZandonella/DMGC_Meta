...
Finally, \textcite{keAlternativeGoalStructures2008a} is the only study where multiple independent treatment groups were compared to multiple independent control groups. In this case the correlation between outcomes is set to zero.
